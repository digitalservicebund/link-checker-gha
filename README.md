  ...
  link-checker:
    ...
    steps:
      - name: Link Checker
        ...
        with:
          SITEMAP_URL: "https://zfl.bund.de/sitemap-1.xml" # <- CHANGE THIS
          SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
```

## Test

To test the workflow locally, use [act](https://github.com/nektos/act):

```sh
act -j link-checker-test --bind
```

With the `--bind` flag, debug files will remain in a local `lychee` dir.
